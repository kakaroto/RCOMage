...
void
rco_fix_decomp_sizes (rRCOFile * rco, rRCOEntry * entry)
{
  if (entry->srcFile[0] && entry->srcCompression) {
	...
	if (unpackedLen != 0xFFFFFFFF) {
	  if (entry->srcLenUnpacked != 0xFFFFFFFF &&
	      unpackedLen != entry->srcLenUnpacked)
	    warning
		("Specified uncompressed size for entry '%s' is incorrect.  Specified length = %d, actual length = %d",
		rco->labels + entry->labelOffset, entry->srcLenUnpacked,
		unpackedLen);

...
rRCOEntry* rco_add_entry(rRCOEntry* parent, rRCOEntry* newEntry, int pos) {
	rRCOEntry* dest;
	if(pos != -1 && pos > (int)(parent->numSubentries))
		return NULL;
	
	if(!parent->numSubentries) {
		parent->subentries = (rRCOEntry*)malloc(sizeof(rRCOEntry));
		dest = parent->subentries;
	} else {
		parent->subentries = (rRCOEntry*)realloc(parent->subentries, sizeof(rRCOEntry) * (parent->numSubentries+1));
		if(pos == -1)
			dest = &(parent->subentries[parent->numSubentries]);
		else {
			// shift things up by one place
			dest = &(parent->subentries[pos]);
			uint numEntriesToMove = parent->numSubentries - pos;
			//rRCOEntry* tmpBuffer = (rRCOEntry*)malloc()
			memcpy(&(parent->subentries[pos+1]), &(parent->subentries[pos]), numEntriesToMove * sizeof(rRCOEntry));
		}
	}
	
	parent->numSubentries++;
	memcpy(dest, newEntry, sizeof(rRCOEntry));
	
	return dest;
}
*/

// parses an RCO (give it the main entry) to fix any resource which is compressed, but uncompressed size is unknown
...
void rco_fix_decomp_sizes(rRCOFile* rco, rRCOEntry* entry) {
	if(entry->srcFile[0] && entry->srcCompression) {
			...
			if(fp) {
				...
				if(unpackedLen != 0xFFFFFFFF) {
					if(entry->srcLenUnpacked != 0xFFFFFFFF && unpackedLen != entry->srcLenUnpacked)
						warning("Specified uncompressed size for entry '%s' is incorrect.  Specified length = %d, actual length = %d", rco->labels + entry->labelOffset, entry->srcLenUnpacked, unpackedLen);
		...
		if(entry->srcLenUnpacked == 0xFFFFFFFF) {
			warning("Uncompressed size for entry '%s' not specified and cannot be automatically determined!  Setting size to 0.", rco->labels + entry->labelOffset);
...
void* read_resource(rRCOEntry* entry, uint* outLen) {
	char* bufferMid;
	if((!entry->srcFile[0] || strchr(entry->srcFile, '*')) && entry->srcBuffer) {
		// this entry should be a sound/text entry (we assume this)
		// also entry->srcAddr should be 0, and entry->srcLen == entry->srcLenUnpacked should be true (and equal the length of the buffer), _and_ entry->srcCompression should be NONE
		...
		*outLen = entry->srcLen;
		...
		if(entry->srcCompression == RCO_DATA_COMPRESSION_RCO) {
			// special case where stuff is embedded in RCO - this should only be used with VSMX, so we'll take a short cut here, but:
			// TODO: do some extra checks to enforce the above
			
			// we assume it's in the tables section (otherwise, there's no point in referencing an RCO
			if(entry->srcAddr < sizeof(PRFHeader)) return NULL;
			PRFHeader header;
			Bool eSwap;
			fileread(src, &header, sizeof(header));
			eSwap = (header.signature == ENDIAN_SWAP_32(RCO_SIGNATURE));
			if(eSwap) es_rcoHeader(&header);
			if(header.compression >> 4) {
				HeaderComprInfo hci;
				fileread(src, &hci, sizeof(hci));
				if(eSwap) es_headerComprInfo(&hci);
				
				// check if the entry _does_ exist in the RCO tree structure
				if(!(entry->srcAddr >= sizeof(header) && entry->srcAddr < sizeof(header)+hci.lenUnpacked && entry->srcAddr+entry->srcLen <= sizeof(header)+hci.lenUnpacked)) return NULL;
				...
				if(header.compression >> 4 == RCO_DATA_COMPRESSION_ZLIB) {
					int uRet = zlib_uncompress(bufferInDecomp, hci.lenUnpacked, bufferIn, hci.lenPacked);
					...
					memcpy(bufferMid, bufferInDecomp+entry->srcAddr-sizeof(header), entry->srcLen);
					free(bufferInDecomp);
			...
			if(entry->srcAddr) fseek(src, entry->srcAddr, SEEK_SET);
			...
			if(entry->srcCompression) {
				...
				if(entry->srcCompression == RCO_DATA_COMPRESSION_ZLIB) {
					int uRet = zlib_uncompress(bufferMid, entry->srcLenUnpacked, bufferIn, entry->srcLen);
					if(uRet != Z_OK && uRet != Z_DATA_ERROR) {
						free(bufferIn);
						free(bufferMid);
						return NULL;
					...
					*outLen = entry->srcLenUnpacked;
					...
					warning("RLZ decompression not supported - data will be left uncompressed.");
					bufferMid = bufferIn;
					*outLen = entry->srcLen;
				} else {
					// unknown compression
				...
				bufferMid = bufferIn;
				// entry->srcLenUnpacked == entry->srcLen SHOULD BE TRUE HERE (we assume this)
				*outLen = entry->srcLen;
			}
		}
	}
...
rRCOEntry** make_sorted_list_of_subentries(rRCOEntry* parent, int(*compar)(const rRCOEntry**, const rRCOEntry**)) {
	...
	rRCOEntry** children = (rRCOEntry**)malloc(parent->numSubentries * sizeof(rRCOEntry*));
	children[0] = parent->firstChild;
	uint i;
	for(i=1; i<parent->numSubentries; i++)
		children[i] = children[i-1]->next;
	
	qsort(children, parent->numSubentries, sizeof(rRCOEntry*), (int(*)(const void*, const void*))compar);
	return children;
}

rRCOEntry* find_entry_from_label(rRCOEntry* parent, const char* s) {
	if(parent->labelOffset != RCO_NULL_PTR && !strcmp(parent->labelOffset + parent->rco->labels, s)) return parent;
	
	rRCOEntry* rcoNode;
	for(rcoNode=parent->firstChild; rcoNode; rcoNode=rcoNode->next) {
		rRCOEntry* res;
		if((res = find_entry_from_label(rcoNode, s))) return res;
	}
...
int find_text_from_label(char* labels, rRCOTextEntry* textExtra, const char* s) {
	if(!textExtra->numIndexes) return -1;
...
void make_iconv_charset(char out[8], int fmt, Bool es) {
	strcpy(out, "ucs-2le");
	if(es) out[5] = 'b'; // big endian
	if(fmt == RCO_TEXT_FMT_UTF32) out[4] = '4';
...
void es_extraObjAnim(Bool isObj, int type, void* data, Bool isPS3) {
	...
	/*
	if(!isObj && RCO_ANIM_EXTRA_REFS[type]) {
		uData[0] = ENDIAN_SWAP_HALF32(uData[0]);
		uData[1] = ENDIAN_SWAP(uData[1]);
		i = 2;
	}
	*/

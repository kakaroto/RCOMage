...
uint zlib_compress(void* src, uint srcLen, void* dest, uint destLen, int level, int strat) {
	if(strat == Z_USE_7Z) {
		...
		*(uint32*)(destPtr + outSize) = ENDIAN_SWAP_32(adler);
		...
		z_stream s;
		memset(&s, 0, sizeof(s));
		if(deflateInit2(&s, level, Z_DEFLATED, 15, 8, strat)) return 0;
		...
		if(deflate(&s, Z_FINISH) < 0) return 0;
...
int zlib_uncompress(void* dest, unsigned int destLen, const void* src, unsigned int srcLen) {
	...
	if(ret == Z_OK) {
		...
		if(inflateEnd(&zs) == Z_DATA_ERROR) ret = Z_DATA_ERROR;
	}
	if(ret == Z_BUF_ERROR) ret = Z_DATA_ERROR;
	return ret;
}
...
uint zlib_unpacked_size(void* src, uint srcLen) {
	...
	if(inflateInit(&s) != Z_OK) return 0xFFFFFFFF;
	...
	inflateEnd(&s);
	if(s.avail_in || (ret != Z_STREAM_END && ret != Z_OK)) return 0xFFFFFFFF;
...
uint rlz_compress(void* src, uint srcLen, void* dest, uint destLen, int mode) {
	if(mode == -1) {
		...
		// we'll be just as bad as the theme creator and run _4_ compression passes >_>
		...
		if((size5 == -1) && (size6 == -1) && (size7 == -1)) return 0; //all failed, lol
		
		if(size7 != -1 && (size7 < size6 || size6 != -1) && (size7 < size5 || size5 != -1))
			return size7; // okay, we're _slightly_ more optimised than the theme creator :P
...
uint find_larger_prime(uint in) {
	...
	if(!(in & 1)) in++; // in is an odd number
	
	while(TRUE) {
		if(is_prime(in)) return in;
		in+=2;
	}
}
Bool is_prime(uint in) {
	if(in < 12) { // need this buffer as "lim" (below) may underflow

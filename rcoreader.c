...
void read_entry(rRCOFile_readhelper* rcoH, rRCOFile* rco, rRCOEntry* data, Bool readSubEntries);
...
void fix_refs(rRCOFile* rco, rRCOEntry* entry, const int* lenArray, const uint lenNum, Bool isObj);
...
rRCOFile* read_rco(char* fn) {
	...
	if(header.null != 0) {
		warning("[header] Unexpected value @ 0x8: 0x%d (expected: 0x0).", header.null);
	}
	
	if(header.pUnknown != RCO_NULL_PTR) {
		warning("[header] Unknown RCO section @ 0x28 (points to 0x%x)", header.pUnknown);
	}
	//if(header.pFontTable != RCO_NULL_PTR) {
	//	warning("[header] Unknown RCO section @ 0x2C (points to 0x%x)", header.pFontTable);
	//}
	if(header.unknown[0] != 0xFFFFFFFF || header.unknown[1] != 0xFFFFFFFF || header.unknown[2] != 0xFFFFFFFF) {
		warning("[header] Unexpected value(s) in range 0x98-0xA4");
	}
	
	rco->umdFlag = header.compression & 0xF; // actually, I'm usure about this, but meh
	...
	
	{
		...
		if(header.lTextData)
			info("    TextData Offset = 0x%x [length = 0x%x]", header.pTextData, header.lTextData);
		if(header.lLabelData)
			info("    NameData Offset = 0x%x [length = 0x%x]", header.pLabelData, header.lLabelData);
		if(header.lEventData)
			info("    EventData Offset = 0x%x [length = 0x%x]", header.pEventData, header.lEventData);
		
		if(header.lImgData)
			info("    ImageData Offset = 0x%x [length = 0x%x]", header.pImgData, header.lImgData);
		if(header.lSoundData)
			info("    SoundData Offset = 0x%x [length = 0x%x]", header.pSoundData, header.lSoundData);
		if(header.lModelData)
			info("    ModelData Offset = 0x%x [length = 0x%x]", header.pModelData, header.lModelData);
	}
	...
	switch(rco->headerCompression) {
		...
		case RCO_DATA_COMPRESSION_ZLIB:
			if(header.version < 0x90)
				warning("[header] Unexpected version 0x%x for compression type", header.version);
			break;
		...
		default:
			error("[header] Unknown compression type 0x%x - process cannot continue.", rco->headerCompression);
			return NULL;
			/* warning("[header] Unknown compression type 0x%x - assuming uncompressed data.", rco->headerCompression);
			// set it to no compression
			rco->headerCompression = RCO_DATA_COMPRESSION_NONE; */
	}
	
	rco->ps3 = (rco->eSwap); //  && header.version >= 0x97
	...
	if(rco->headerCompression) {
		...
		if(ci.lenUnpacked > MAX_TREE_DATA || ci.lenPacked > MAX_TREE_DATA) {
			error("[header] Size of tree data exceeds sane limit.  This is probably a bad RCO.");
			return NULL;
		}
		...
		switch(rco->headerCompression) {
			case RCO_DATA_COMPRESSION_ZLIB:
				{
					// uses a bit of memory... - could use zlib's inflate routines, but eh, I doubt it really matters that much...
					void* readBuf = malloc(ci.lenPacked);
					fileread(rcoH.fp, readBuf, ci.lenPacked);
					int uRet = zlib_uncompress(rcoH.tables, ci.lenUnpacked, readBuf, ci.lenPacked);
					if(uRet != Z_OK && uRet != Z_DATA_ERROR) {
						error("[entries] Unable to decompress tree entries!");
						return FALSE;
					} else if(uRet == Z_DATA_ERROR) {
						warning("Encountered 'data error' when decompressing tree entries.");
					}
					free(readBuf);
			...
			default: // this won't actually ever be executed due to the new compression checking code above... :/
				error("[header] Unknown compression method specified (0x%x) - can't continue.", rco->headerCompression);
				return NULL;
		}
		...
		if(header.pTextData != RCO_NULL_PTR && header.lTextData) {
			fseek(rcoH.fp, header.pTextData, SEEK_SET); // TODO: check offset first!
			TextComprInfo tci;
			...
			do {
				...
				if(rco->eSwap) es_textComprInfo(&tci);
				
				if(tci.unknown != 0x1) warning("[text-data] Unexpected value (0x%x) @ 0x2 - expected 0x1", tci.unknown);
				...
				int uRet = zlib_uncompress((char*)rcoH.tables + oldSize, tci.unpackedLen, readBuf, tci.packedLen);
				if(uRet != Z_OK && uRet != Z_DATA_ERROR) {
				...
				rcoH.fSizeExpanded += ALIGN_TO_4(tci.unpackedLen) - ALIGN_TO_4(tci.packedLen); // TODO: need to check if this is correct
				
				if(!tci.nextOffset) break;
	...
	if(!check_file_region(rcoH.fSizeExpanded, header.pLabelData, header.lLabelData)) {
		error("[header] Invalid label pointer/length specified.");
		return NULL;
	}
	if(header.lLabelData > MAX_LABEL_DATA) {
		header.lLabelData = MAX_LABEL_DATA;
		warning("[labels] Total data length exceeds safety limit of 16MB - data has been truncated!");
	}
	...
	if(!check_file_region(rcoH.fSizeExpanded, header.pEventData, header.lEventData)) {
		error("[header] Invalid event pointer/length specified.");
		return NULL;
	}
	if(header.lEventData > MAX_LABEL_DATA) {
		warning("[events] Total data length (%d) exceeds safety limit of 16MB - data has been truncated!", header.lEventData);
	...
	// TODO: think about what to do with these
	{
		...
		/*
		READ_RCO_READ_PTR_SEGMENT(header.pTextPtrs, header.lTextPtrs, rcoH.ptrsText, rcoH.numTextPtrs, "text");
		READ_RCO_READ_PTR_SEGMENT(header.pImgPtrs, header.lImgPtrs, rcoH.ptrsImg, rcoH.numImgPtrs, "image");
		READ_RCO_READ_PTR_SEGMENT(header.pModelPtrs, header.lModelPtrs, rcoH.ptrsModel, rcoH.numModelPtrs, "model");
		READ_RCO_READ_PTR_SEGMENT(header.pSoundPtrs, header.lSoundPtrs, rcoH.ptrsSound, rcoH.numSoundPtrs, "sound");
		*/
		READ_RCO_READ_PTR_SEGMENT(header.pObjPtrs, header.lObjPtrs, rcoH.ptrsObj, rcoH.numObjPtrs, "object");
		READ_RCO_READ_PTR_SEGMENT(header.pAnimPtrs, header.lAnimPtrs, rcoH.ptrsAnim, rcoH.numAnimPtrs, "anim");
		
		// TODO: possibly sort these for the following section (not that necessary - only useful if we find an unknown object/anim type)
	...
	{
		rco->tblVSMX = rco->tblText = rco->tblImage = rco->tblSound = rco->tblModel = rco->tblFont = rco->tblObj = rco->tblAnim = 0;
		rRCOEntry* rcoNode;
		for(rcoNode = rco->tblMain.firstChild; rcoNode; rcoNode = rcoNode->next) {
			switch(rcoNode->id) {
				#define READ_RCO_ASN_SHORTCUT_TBL(hp, sc, s) \
					if(hp == RCO_NULL_PTR) warning("[header] %c%s main tree not referenced but found in entries.", toupper(s[0]), s+1); \
				...
				case RCO_TABLE_SOUND:
					READ_RCO_ASN_SHORTCUT_TBL(header.pSoundTable, rco->tblSound, "sound");
					break;
				case RCO_TABLE_MODEL:
					READ_RCO_ASN_SHORTCUT_TBL(header.pModelTable, rco->tblModel, "model");
					break;
				case RCO_TABLE_FONT:
				...
				case RCO_TABLE_OBJ:
					...
					break;
				case RCO_TABLE_ANIM:
					READ_RCO_ASN_SHORTCUT_TBL(header.pAnimTable, rco->tblAnim, "animation");
					break;
			}
		}
		
		if(header.pVSMXTable != RCO_NULL_PTR && !rco->tblVSMX) warning("[header] VSMX tree referenced in header, but couldn't be found when parsing entries!  Section has been discarded.");
		if(header.pTextTable != RCO_NULL_PTR && !rco->tblText) warning("[header] Text tree referenced in header, but couldn't be found when parsing entries!  Section has been discarded.");
		if(header.pImgTable != RCO_NULL_PTR && !rco->tblImage) warning("[header] Image tree referenced in header, but couldn't be found when parsing entries!  Section has been discarded.");
		if(header.pSoundTable != RCO_NULL_PTR && !rco->tblSound) warning("[header] Sound tree referenced in header, but couldn't be found when parsing entries!  Section has been discarded.");
		if(header.pModelTable != RCO_NULL_PTR && !rco->tblModel) warning("[header] Model tree referenced in header, but couldn't be found when parsing entries!  Section has been discarded.");
		if(header.pFontTable != RCO_NULL_PTR && !rco->tblFont) warning("[header] Font tree referenced in header, but couldn't be found when parsing entries!  Section has been discarded.");
		if(header.pObjTable != RCO_NULL_PTR && !rco->tblObj) warning("[header] Object tree referenced in header, but couldn't be found when parsing entries!  Section has been discarded.");
		if(header.pAnimTable != RCO_NULL_PTR && !rco->tblAnim) warning("[header] Animation tree referenced in header, but couldn't be found when parsing entries!  Section has been discarded.");
	}
	...
	if(rco->tblText && header.lTextData) {
		...
		if(rco->headerCompression != RCO_DATA_COMPRESSION_NONE) {
			// if header is compressed, we've got to reparse the RCO file to get the positions :/
			...
			if(rco->eSwap) es_headerComprInfo(&hci);
			fseek(fp2, ALIGN_TO_4(hci.lenPacked), SEEK_CUR);
			
			
			// before parsing the file, we'll be on the safe side and blank out all text entries' src
	...
	// TODO: really should put in the right values when traversing the tree instead of a post-fix
	{
		rRCOEntry* rcoNode;
		if(rco->tblImage) {
	...
	if(rco->tblAnim)
		fix_refs(rco, rco->tblAnim, RCO_ANIM_EXTRA_LEN, RCO_ANIM_EXTRA_LEN_NUM, FALSE);
	...
	// check resources
	{
		// TODO: should use rcoH.fSizeExpanded here otherwise we stuff up the region tracking code
		...
		/*
		rco->pDataText = header.pTextData;
		rco->lDataText = header.lTextData;
		if(!rco->lDataText) rco->pDataText = 0;
		rco->pDataImg = header.pImgData;
		rco->lDataImg = header.lImgData;
		if(!rco->lDataImg) rco->pDataImg = 0;
		rco->pDataSound = header.pSoundData;
		rco->lDataSound = header.lSoundData;
		if(!rco->lDataSound) rco->pDataSound = 0;
		rco->pDataModel = header.pModelData;
		rco->lDataModel = header.lModelData;
	...
	fclose(rcoH.fp);
	if(rcoH.tables) free(rcoH.tables);
	
	/* if(rcoH.ptrsText) free(rcoH.ptrsText);
	if(rcoH.ptrsImg) free(rcoH.ptrsImg);
	if(rcoH.ptrsModel) free(rcoH.ptrsModel);
	if(rcoH.ptrsSound) free(rcoH.ptrsSound); */
	if(rcoH.ptrsObj) free(rcoH.ptrsObj);
	if(rcoH.ptrsAnim) free(rcoH.ptrsAnim);
...
void read_entry(rRCOFile_readhelper* rcoH, rRCOFile* rco, rRCOEntry* data, Bool readSubEntries) {
	...
	if(re.labelOffset != RCO_NULL_PTR) {
		if(re.labelOffset >= rco->labelsLen) {
			warning("[entry (0x%x)] Invalid label offset supplied - label removed", data->offset);
		} else {
			// WARNING! this assignment is unchecked (ie possible to cause this to point to anything)
			data->labelOffset = re.labelOffset;
		}
	}
	if(re.eHeadSize && re.eHeadSize != sizeof(re)) {
		warning("[entry (0x%x)] Non-standard entry header size found!", data->offset);
	}
	
	data->numSubentries = re.numSubentries;
	data->rco = rco;
	data->parent = data->firstChild = data->lastChild = data->prev = data->next = NULL;
	if(re.blanks[0] != 0 || re.blanks[1] != 0) {
		warning("[entry (0x%x)] Unexpected entry value in range 0x20-0x28", data->offset);
	}
	...
	uint extraSize = 0;
	{
		...
		switch(data->id) {
			case RCO_TABLE_MAIN:
				if(data->type != 1)
					warning("[entry (0x%x)] Unexpected 'main' type (0x%x)!", data->offset, data->type);
			...
			case RCO_TABLE_VSMX:
				if(data->type == 1) {
					...
					if(rco->eSwap) es_rcoVsmxEntry(&rve);
					data->srcAddr = rcoread_ftell(rcoH) + rve.offset;
					data->srcLen = data->srcLenUnpacked = rve.length;
					strcpy(data->srcFile, rcoH->fName);
					// need to consider possibility that VSMX table is compressed inside compressed RCO header
					data->srcCompression = (rcoH->tables ? RCO_DATA_COMPRESSION_RCO : RCO_DATA_COMPRESSION_NONE);
					
					
					extraSize = sizeof(RCOVSMXEntry) + rve.length;
					// 4 byte alignment
					extraSize = ALIGN_TO_4(extraSize);
					rcoread_fseek(rcoH, rcoread_ftell(rcoH) + extraSize - sizeof(RCOVSMXEntry));
				} else
					warning("[entry (0x%x)] Unknown VSMX type (0x%x)!", data->offset, data->type);
			...
			case RCO_TABLE_TEXT:
				if(data->type == 1) {
					...
					if(rco->eSwap) es_rcoTextEntry(&rte);
					
					dest->lang = rte.lang;
					dest->format = rte.format;
					if(rte.format != RCO_TEXT_FMT_UTF8 && rte.format != RCO_TEXT_FMT_UTF16 && rte.format != RCO_TEXT_FMT_UTF32) {
						warning("[entry (0x%x)] Unknown format for text entry (0x%x)", data->offset, rte.format);
					}
					...
					if(dest->numIndexes > MAX_SUBENTRIES) {
						warning("[entry (0x%x)] Number of text indexes (0x%x) exceeds safety limit.", data->offset, ((rRCOTextEntry*)data->extra)->numIndexes);
					...
					strcpy(data->srcFile, rcoH->fName);
					// special case for text - source address/length must be updated later
					
				} else if(data->type != 0)
					warning("[entry (0x%x)] Unknown text type (0x%x)!", data->offset, data->type);
				break;
			...
			case RCO_TABLE_MODEL:
				if(data->type == 1) {
					...
					uint rimeSize = (rco->ps3 ? sizeof(RCOPS3ImgModelEntry) : sizeof(RCOImgModelEntry));
					uint16 compression;
					extraSize = rimeSize;
					
					// check for those certain entries which aren't compressed and don't have a packed size value
					// urgh, this is a bit of an ugly hack - these short entries are something I didn't know about before :/
					
					if(rco_fread(rcoH, &rie, sizeof(uint32))) {
						compression = rie.compression;
						if(rco->eSwap) compression = ENDIAN_SWAP(compression);
						compression &= 0xFF;
						if(compression == RCO_DATA_COMPRESSION_NONE) {
							// assume that this doesn't include the packed length value
							// ...but we'll still at least _try_ to see if this isn't a short entry (not guaranteed to succeed, especially for the last image entry)
							if(!re.nextEntryOffset || re.nextEntryOffset < sizeof(RCOEntry) + rimeSize)
					...
					if(rco->ps3) {
						if(!rco_fread(rcoH, (uint8*)(&rie) +sizeof(uint32), sizeof(uint32)*3)) {
							error("[entry (0x%x)] Unable to read entry extra data - file exhausted.", data->offset);
							return;
						}
						if(rie.sizeUnpacked != ENDIAN_SWAP_32(1))
							warning("[entry (0x%x)] Unexpected value 0x%x for PS3 image - expected 0x1.", data->offset, ENDIAN_SWAP(rie.sizeUnpacked));
						rie.sizeUnpacked = rie.sizePacked;
						if(compression) {
							rco_fread(rcoH, &rie.sizeUnpacked, sizeof(uint32));
						}
					}
					else if(!rco_fread(rcoH, (uint8*)(&rie) +sizeof(uint32), extraSize - sizeof(uint32))) {
						error("[entry (0x%x)] Unable to read entry extra data - file exhausted.", data->offset);
						return;
					}
					
					/*
					// dirty hack for reading PS3 stuff
					if(rco->ps3 && extraSize == rimeSize) extraSize -= sizeof(uint32);
					
					if(!rco_fread(rcoH, (uint8*)(&rie) +sizeof(uint32), extraSize - sizeof(uint32))) {
						error("[entry (0x%x)] Unable to read entry extra data - file exhausted.", data->offset);
						return;
					}
					if(rco->ps3 && rie.sizeUnpacked != ENDIAN_SWAP_32(1))
						warning("[entry (0x%x)] Unexpected value 0x%x for PS3 image - expected 0x1.", data->offset, ENDIAN_SWAP(rie.sizeUnpacked));
					if(rco->ps3 && compression && extraSize + sizeof(uint32) == rimeSize) {
						extraSize += sizeof(uint32);
						rco_fread(rcoH, &rie.sizeUnpacked, sizeof(uint32));
						// TODO: (fix this?) ignore the weird unknown entry for now
					}
					*/
					if(rco->eSwap) es_rcoImgModelEntry(&rie);
					...
					((rRCOImgModelEntry*)data->extra)->compression = rie.compression & 0xFF;
					((rRCOImgModelEntry*)data->extra)->unkCompr = rie.compression >> 8;
				} else if(data->type != 0)
					warning("[entry (0x%x)] Unknown image/model type (0x%x)!", data->offset, data->type);
				break;
			...
			case RCO_TABLE_SOUND:
				if(data->type == 1) {
					...
					if(rco->eSwap) es_rcoSoundEntry(&rse);
					strcpy(data->srcFile, rcoH->fName);
					...
					((rRCOSoundEntry*)data->extra)->channelData = (uint32*)malloc(readAmt);
					if(!rco_fread(rcoH, ((rRCOSoundEntry*)data->extra)->channelData, readAmt)) {
						error("[entry (0x%x)] Unable to read entry extra data - file exhausted.", data->offset);
						return;
					}
					uint i;
					...
					for(i=0; i<rse.channels; i++)
						((rRCOSoundEntry*)data->extra)->channelData[i*2+1] -= rse.offset;
					
					if(rse.channels < 2) {
						// check if we need to skip some stuff
						// AMENDMENT: actually, it seems that there _must_ be two channels defined (no clear indication of size otherwise)
						for(i=rse.channels; i<2; i++) {
							struct {uint32 size; uint32 offset;} p;
							...
							if(p.size || p.offset != RCO_NULL_PTR)
								warning("[entry (0x%x)] Unexpected values found in sound entry data where null channels were expected.", data->offset);
							
							extraSize += sizeof(uint32)*2;
						}
					}
				} else if(data->type != 0)
					warning("[entry (0x%x)] Unknown sound type (0x%x)!", data->offset, data->type);
				break;
			...
			case RCO_TABLE_FONT:
				if(data->type == 1) {
					...
					if(rco->eSwap) es_rcoFontEntry(&rfe);
					...
					warning("[entry (0x%x)] Unknown Font type (0x%x)!", data->offset, data->type);
			...
			case RCO_TABLE_OBJ:
				if(data->type > 0) {
					...
					if(data->type <= (int)RCO_OBJ_EXTRA_LEN_NUM && RCO_OBJ_EXTRA_LEN[data->type] != -1) {
						entrySize = RCO_OBJ_EXTRA_LEN[data->type]*4;
					}
					else
						warning("[entry (0x%x)] Unknown object type (0x%x)!", data->offset, data->type);
					
					if(re.entrySize) {
						if(entrySize) {
							if(entrySize != re.entrySize - sizeof(re))
								warning("[entry (0x%x)] Entry length does not match standard length for this type of object!", data->offset);
						}
					...
					if(entrySize) {
						...
						if(rco->eSwap) es_extraObjAnim(TRUE, data->type, data->extra, rco->ps3);
					} else {
						error("[entry (0x%x)] Unable to determine object entry length!", data->offset);
						return;
					}
				}
			...
			case RCO_TABLE_ANIM:
				if(data->type > 1) {
					...
					if(data->type <= RCO_ANIM_EXTRA_LEN_NUM && RCO_ANIM_EXTRA_LEN[data->type] != -1)
						entrySize = RCO_ANIM_EXTRA_LEN[data->type]*4;
					else
						warning("[entry (0x%x)] Unknown animation type (0x%x)!", data->offset, data->type);
					
					if(re.nextEntryOffset) {
						if(entrySize) {
							if(entrySize != re.nextEntryOffset - sizeof(re))
								warning("[entry (0x%x)] Entry length (%d) does not match standard length (%d) for this type of animation action!", data->offset, re.nextEntryOffset-sizeof(re), entrySize);
						}
					...
					if(entrySize) {
						READ_ENTRY_MALLOC_AND_READ(entrySize);
						if(rco->eSwap) es_extraObjAnim(FALSE, data->type, data->extra, rco->ps3);
					} else {
						error("[entry (0x%x)] Unable to determine anim entry length!", data->offset);
						return;
					}
				}
				data->extraLen = extraSize;
	...
	if(readSubEntries && data->numSubentries) {
		...
		if(data->numSubentries > MAX_SUBENTRIES) {
			warning("[entry (0x%x)] Number of subentries (%d) exceeds safety limit of 65536!", data->offset, data->numSubentries);
	...
	// check if lengths match - note, last entry in list won't have a next entry offset so will cause a misalignment for the parent
	if(re.nextEntryOffset && re.nextEntryOffset != sizeof(re)+extraSize) {
		warning("[entry (0x%x)] Entry boundaries not aligned (going forward %d bytes)", data->offset, re.nextEntryOffset - (sizeof(re)+extraSize));
		rcoread_fseek(rcoH, data->offset + re.nextEntryOffset);
		
		// our ugly hack to get around borked update_plugin.rco when decompressed with Resurssiklunssi
		// the issue is that the image isn't compressed by default, so uses a short image entry (ie, doesn't include decompressed size), however Resurssiklunssi will compress it but won't add in this decompressed size since there is no room for it
		// rcomage expects a decompressed size so will assume it exists (no way to tell if it's not there) but this will cause an alignment error for the parent.  The only way to detect this is, thus, trap this alignment error and check the child entry
		// still, we are left without a decompressed size, thus we have to figure out something for this
		
		// we'll do this by seeing if the misalignment is -4 and the previous entry is a compressed image/model; if there are child entries, we'll check this instead of the previous entry
		// obviously this isn't guaranteed to work in 100% of cases, but will get around the update_plugin.rco bug
		if(re.nextEntryOffset == sizeof(re)+extraSize -4) {
			if(data->lastChild) {
				if(data->lastChild->srcFile[0] && data->lastChild->srcCompression && data->lastChild->type == 1 && (data->lastChild->id == RCO_TABLE_IMG || data->lastChild->id == RCO_TABLE_MODEL)) {
...
// linear/recursive search - not the fastest, but eh, performance isn't that much of an issue
rRCOEntry* find_entry_from_offset(rRCOEntry* parent, uint32 offset) {
	if(parent->offset == offset) return parent;
...
void fix_refs(rRCOFile* rco, rRCOEntry* entry, const int* lenArray, const uint lenNum, Bool isObj) {
	...
	// only fix refs if type is known, and not the main object(0x800)/anim(0x900) table
	if(entry->type != 0 && entry->type <= (int)lenNum && lenArray[entry->type] != -1) {
			...
			if(cond) {
				...
				switch(ref->type) {
					...
					case RCO_REF_TEXT:
						...
						// TODO: because of the above, we should probably check that all the labels for all text data are the same (though this app will work fine without it, except for XML writing/reading)
						break;
					case RCO_REF_IMG: case RCO_REF_MODEL: case RCO_REF_FONT: case RCO_REF_OBJ2: case RCO_REF_ANIM: case RCO_REF_OBJ:
					// TODO: consider only searching specific sections instead of the entire table
					newRef->ptr = find_entry_from_offset(&(rco->tblMain), ref->ptr);
					if(!newRef->ptr)
						warning("[entry (0x%x)] Unable to find referenced entry from supplied pointer.", entry->offset);
					break;
					case RCO_REF_NONE: 
						if(ref->ptr != RCO_NULL_PTR)
							warning("[entry (0x%x)] Unexpected pointer value for null pointer (expected 0xFFFFFFFF but got 0x%x).", entry->offset, ref->ptr);
						break;
					default:
						warning("[entry (0x%x)] Unknown reference type 0x%x.", entry->offset, ref->type);
				}
				i++;

...
#define READ_ENTRY_MALLOC_AND_READ(s) { \
	...
	if (data->type > 1) {
	  ...
	  if (re.nextEntryOffset) {
		...
		warning
		    ("[entry (0x%x)] Entry length (%d) does not match standard length (%d) for this type of animation action!",
		    data->offset, re.nextEntryOffset - sizeof (re), entrySize);
	    }
	...
	break;
      ...
      default:
	warning ("[entry (0x%x)] Unknown entry type (0x%x)", data->offset,
	    data->id);
    }
  ...
  if (re.nextEntryOffset && re.nextEntryOffset != sizeof (re) + extraSize) {
	...
	data->offset, re.nextEntryOffset - (sizeof (re) + extraSize));

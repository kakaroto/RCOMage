...
#define scast_405A10(v) ((long double)((unsigned int)(v)))
      ...
      rlzi->sgl_120C84 +=
	  (float) (log2 (scast_405A10 (oldRangeSize) /
							...
							// rlzcompress())
{
  ...
  for (i = 0; i < 8; i++) {
    ...
    write_bit ((unsigned char *) ((unsigned int) &rlzi->lenDesc[i * 8] + arg_4),
	flagTemp);
    if (flagTemp == 0)
      break;
  }
...
flush_output (void)
{
  ...
  if ((rlzi->rangeOffset >> 0x18) != 0xFF) {
	...
	return;
      *(unsigned char *) ((unsigned int) rlzi->output + rlzi->outputpos) =
...
#define ROUND(n) ((int)((n) + 0.5))
  ...
  while (backRef != -1) {	// found a back reference
    ...
    for (; matchAmt < maxBlockSize; matchAmt++) {
      // loc_405E70:
      if (*(unsigned char *)
	  (((unsigned int) rlzi->input + matchAmt) + backRef) !=
	  *(unsigned char *) (((unsigned int) rlzi->input + matchAmt) +
	      rlzi->inputpos))
	break;
    }
	...
	*searchBackUsed = i;
	if (*searchBackUsed) {
	  ...
	  for (i = 0; i < matchAmt; i++) {
	    if (*(unsigned char *)
		((unsigned int) rlzi->input + backRef) !=
		*(unsigned char *) (ABS_INPUT_POS + i))
	      break;
	  }
